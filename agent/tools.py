...
def init_db():
    ...
    用 INSERT OR IGNORE：只在缺失时补种子数据，不会覆盖运行时修改。
    """
    with closing(sqlite3.connect(DB_PATH)) as conn:
        ...
        """)
        c.execute("INSERT OR IGNORE INTO cities VALUES ('北京', '北京市', 2189)")
        c.execute("INSERT OR IGNORE INTO cities VALUES ('上海', '上海市', 2487)")
        c.execute("INSERT OR IGNORE INTO cities VALUES ('广州', '广东省', 1881)")
        c.execute("INSERT OR IGNORE INTO cities VALUES ('深圳', '广东省', 1766)")
        c.execute("INSERT OR IGNORE INTO cities VALUES ('杭州', '浙江省', 1237)")
        c.execute("INSERT OR IGNORE INTO cities VALUES ('成都', '四川省', 2126)")
        ...
        """)
        c.execute("INSERT OR IGNORE INTO users VALUES ('1001', '张三', '北京')")
        c.execute("INSERT OR IGNORE INTO users VALUES ('1002', '李四', '上海')")
        c.execute("INSERT OR IGNORE INTO users VALUES ('1003', '王五', '广州')")
...
def validate_args(name: str, args: dict) -> tuple[bool, str]:
    ...
    if extra:
        return False, f"存在多余参数：{', '.join(sorted(extra))}"
